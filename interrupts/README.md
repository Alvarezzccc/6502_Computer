...
When working with interruptions, we are talking about electrical signals arriving at the CPU. In this case, we will be incrementing a counter in RAM (at the begining, initilized at 0) whenver an interrupt is received by the IRQ signal. Then printing it in the LCD screen. We are making use of the binary to decimal algorithm described below from another program already done.
...
vasm -Fbin -dotdir interrupt_counter.s
